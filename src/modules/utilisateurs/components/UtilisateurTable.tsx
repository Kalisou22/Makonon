  ...
  const columns = [
    {
      key: 'nom',
      header: 'Nom',
      render: (item: Utilisateur) => <span className="font-medium">{item.nom}</span>,
    },
    {
      key: 'email',
      header: 'Email',
      render: (item: Utilisateur) => item.email,
    },
    {
      key: 'telephone',
      header: 'Téléphone',
      render: (item: Utilisateur) => item.telephone || '-',
    },
    {
      key: 'role',
      header: 'Rôle',
      render: (item: Utilisateur) => <span className={`px-2 py-1 rounded-full text-xs font-medium ${getRoleColor(item.role)}`}>{getRoleLabel(item.role)}</span>,
      align: 'center' as const,
    },
    {
      key: 'agence',
      header: 'Agence',
      render: (item: Utilisateur) => item.agence?.nom || '-',
    },
    {
      key: 'actif',
      header: 'Statut',
      render: (item: Utilisateur) => <StatusBadge status={item.actif ? 'Actif' : 'Inactif'} variant={item.actif ? 'success' : 'danger'} />,
      align: 'center' as const,
    },
    {
      key: 'actions',
      ...
      render: (item: Utilisateur) => (
        <div className="flex gap-2">
          ...
          <Button variant="danger" size="sm" onClick={() => onDelete(item.id)} disabled={item.role === 'SUPERADMIN'}>Supprimer</Button>
        </div>
      ),
      align: 'center' as const,
    },
