...
class LedgerServiceV2
{
    ...
    public function verifierDoubleEcriture(?int $transactionId): void
    {
        ...
        $totalDebit = Ledger::where('transaction_id', $transactionId)->where('type', 'DEBIT')->sum('montant');
        $totalCredit = Ledger::where('transaction_id', $transactionId)->where('type', 'CREDIT')->sum('montant');
